...
from ..layers.position_encoding import PosPool_abs, PosPool_rel
from ..layers.channel_attention import SELayer_global, pt_global, GCLayer_global
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        if not is_head:
            ...
            self.ab_pos = PosPool_abs(in_channels, mode=4, norm_args=norm_args, act_args=act_args)
            self.rel_pos = PosPool_rel(in_channels, mode=4, norm_args=None, act_args=act_args)
        else:
            self.ab_p = PosPool_abs(out_channels, mode=4, norm_args=None, act_args=act_args)
    ...
    def forward(self, px):
        ...
        if self.is_head:
            ...
            #xj = self.rel_pos(dp, xj)

            #x = self.pool(self.convs(xj))
...
class PointNextDecoder(nn.Module):
    def __init__(self,
        ...
        self.out_channels = fp_channels[-n_decoder_stages]*15
        #self.se = SELayer_global(960, 8, norm_args=None, act_args=act_args)
        self.se = SELayer_global(480, 4, norm_args=None, act_args=act_args)
        #self.pt = pt_global(480, 8, norm_args=None, act_args=act_args)
        #self.gc = GCLayer_global(480, 4, norm_args=norm_args, act_args=act_args)
    ...
    def forward(self, p, f):
        ...
        # f[3]:b,128,n/16
        # f[4]:b,256,n/32
        f[2] = three_interpolation(p[1], p[2], f[2])
        f[3] = three_interpolation(p[1], p[3], f[3])
        f[4] = three_interpolation(p[1], p[4], f[4])
        out = torch.cat((f[1], f[2], f[3], f[4]),dim=1)
        out = self.se(out)
        #out = self.pt(out, p[0])
        #out = self.gc(out)
        return out
