...
def generation(labels, num_cls):
    length= len(labels)
    positive_sit = np.zeros([num_cls, length])
    for i in range(length):
        positive_sit[int(labels[i]), i]=1
    return positive_sit
...
class S3DIS(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split=='train':
            positive_sit = generation(label, 13)
            label = label.squeeze(-1).astype(np.long)
            data = {'pos': coord, 'x': feat, 'y': label, 'sit':positive_sit}
        else:
            label = label.squeeze(-1).astype(np.long)
